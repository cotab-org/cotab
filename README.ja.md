...
- **最重要事項**
  ...
  - **llama-serverを使う場合必ず"-np 1"オプションを指定してください。**
    llama-serverの2025年末の更新にて、デフォルトで4並列で動作するように変更されています。デフォルトのllama-serverの場合、Cotabでは高速でリクエストとキャンセルを繰り返すため、全く別のリクエストと誤認され、プロンプトキャッシュが機能せずに著しいパフォーマンス低下を引き起こします。
