...
- llama-server
  ...
  您也可以使用OpenAI兼容的API，但我们强烈建议使用llama-server。llama-server开销低，在使用llama.cpp作为后端的服务器中高速运行。[详细信息](#使用远程服务器)
...
- **最重要事项**
  ...
  - **使用llama-server时，请务必指定"-np 1"选项。**
    llama-server在2025年末的更新中，已更改为默认以4个并行进程运行。对于默认的llama-server，由于Cotab会高速频繁地重复请求和取消，它们会被误认为是完全不同的请求，导致提示缓存无法正常工作，从而造成显著的性能下降。

  - **还请指定"-b 512"选项。**
    对于RTX 4070等常见的NVIDIA游戏GPU，即使超过512，性能也几乎不会改变。由于llama-server的取消请求在批处理期间不会被接受，使用默认的2048时，可能需要几秒钟才能执行取消，从而导致意外的响应下降。
...
### 模型名称中的Q4和Q2是什么意思？
它们表示量化位数。量化是一种通过降低模型精度来减小文件大小和内存使用量的技术。Q4表示4位量化，Q2表示2位量化。数值越小，文件大小和内存使用量越少，但模型质量也会降低。一般来说，Q4模型在数据大小和质量之间具有最佳平衡。

### Qwen3-Coder-30B-A3B:Q2的质量下降程度如何？
对于Qwen3-Coder-30B-A3B:Q2，不会发生频繁语法错误等严重退化。考虑到它在低VRAM环境中的性能和精度优势，我们建议您实际使用并尝试。

### 补全能否超出视图范围或避免覆盖现有代码？
VS Code的公共API不允许像Cursor那样显示超出编辑器视图的内容。它也不允许在行之间添加任意块。GitHub Copilot使用未发布的内部功能来实现这一点，因此常规插件无法实现与GitHub Copilot同等的用户体验。

### 什么是令牌（Token）？
令牌是LLM处理文本时的基本单位。例如，"Hello"大约为1个令牌。对于代码，令牌数大约是字符数的三分之一。
