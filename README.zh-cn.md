...
它具有高指令遵循性能，并且比Qwen3-4B-Instruct-2507使用更少的VRAM运行，但不推荐使用，因为在Cotab中经常生成质量差的补全。

- LFM2-2.6B

它设计为仅使用VRAM 3GB即可以超过Qwen3-4B-Instruct-2507两倍以上的速度运行。尽管如此，它具有高指令遵循性能，但不推荐使用，因为在Cotab的代码补全中观察到代码损坏的情况。当VRAM要求非常严格或用于翻译目的时可以使用。
