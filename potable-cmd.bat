...
if "%ENABLE_NODEJS%"      equ "" set ENABLE_NODEJS=1
if "%ENABLE_CHECKOUT_COTAB%"     equ "" set ENABLE_CHECKOUT_COTAB=1
...
if "%ENABLE_CHECKOUT_COTAB%" equ "1" (
    call :CHECKOUT_COTAB
    if ERRORLEVEL 1 goto :ERROR
)
...
:CHECKOUT_COTAB
    echo;
    echo ##### checking checkout cotab...
    set COTAB_NAME=cotab
    set COTAB_REPO=%CUR_DIR%
    set COTAB_CHECK_FILE=%COTAB_REPO%\.git
    set COTAB_URL=https://github.com/cotab-org/cotab.git
    
    :: backslash to slash and remove last slash
    set "COTAB_REPO=%COTAB_REPO:\=/%"
    if "%COTAB_REPO:~-1%"=="/" set "COTAB_REPO=%COTAB_REPO:~0,-1%"
    
	if not exist "%COTAB_CHECK_FILE%" (
	    echo # checking out %COTAB_NAME%...
        
        git -C "%COTAB_REPO%" init -b main
		if ERRORLEVEL 1 (
			echo failed to init "%COTAB_NAME%"
			exit /b 1
		)

        git -C "%COTAB_REPO%" remote add origin %COTAB_URL%
		if ERRORLEVEL 1 (
			echo failed to remote "%COTAB_NAME%"
			exit /b 1
		)
        git -C "%COTAB_REPO%" fetch
		if ERRORLEVEL 1 (
			echo failed to fetch "%COTAB_NAME%"
			exit /b 1
		)
        git -C "%COTAB_REPO%" checkout --no-overwrite -t origin/main
		if ERRORLEVEL 1 (
	...
	if exist "%COTAB_CHECK_FILE%" (
	    echo # checkout successfully %COTAB_NAME%...
    )
exit /b 0
